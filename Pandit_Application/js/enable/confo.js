...
$("#disconnect_call").on("click", function () {
    var hdncharges_call_per_minu = $("#hdncharges_call_per_minu").val();
    ...
    room.cancelOutboundCall('918789635170', function (resp) {
        ...
        var res = resp.result;
        if (res.msg == "Success") {
            calculatededuction(moneydeducted);
        /* room.disconnect();*/
...
function disconnect(moneydeducted) {
    debugger
    var mobileno = $("#hdnMobile").val();

    room.cancelOutboundCall(mobileno, function (resp) {
        debugger
        var res = resp.result;
        if (res.msg == "Success") {
...
function display_c() {
    ...
    var time = hdnCustomerwalletamount / hdncharges_call_per_minu;
    var moneydeducted = time * hdncharges_call_per_minu;
    var refresh = 60000 * time; // Refresh rate in milli seconds
    mytime = setTimeout(function () {
        disconnect(moneydeducted)
    }, refresh)
}

function calculatededuction(moneydeducted) {
    $.ajax({
        type: "GET",
        url: '/CallCustomer/Deductedamount/',
        data: { "moneydeducted": moneydeducted },
